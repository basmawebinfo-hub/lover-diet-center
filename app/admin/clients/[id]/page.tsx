...
export default function AdminClientDetailPage() {
  ...
  useEffect(() => { adminFetchClientDetail(id).then((d) => { setData(d); setLoaded(true) }) }, [id])
  useEffect(() => { fetchMeals().then((m) => setMeals(m.map((x) => ({ id: x.id, nameEn: x.nameEn, nameAr: x.nameAr })))) }, [])

  const weightLogs = useMemo(
    () => (data?.weightLogs ?? []).map((w) => ({ id: w.date, date: w.date, weightKg: w.weightKg })),
    [data],
  )

  // --- derived values (safe even if data null) ---
  ...
  const latestWater = data && data.waterLogs.length ? data.waterLogs[data.waterLogs.length - 1].liters : 0
  ...
  function exportCsv() {
    ...
    if (!d) return
    const rows: string[][] = [
      ["Field", "Value"], ["Name", nameEn], ["Email", String(get("email") ?? "")], ["Phone", phone],
      ["Age", String(get("age") ?? "")], ["Gender", String(get("gender") ?? "")], ["Height (cm)", String(get("height_cm") ?? "")],
      ["Start weight", String(start)], ["Current weight", String(current)], ["Target weight", String(target)],
      ["Goal", String(get("goal") ?? "")], ["Orders", String(d.orders.length)], ["Sessions", String(d.sessions.length)],
      ["", ""], ["Weight date", "Weight (kg)"], ...d.weightLogs.map((w) => [w.date, String(w.weightKg)]),
    ]
    const csv = rows.map((r) => r.map((c) => `"${String(c).replace(/"/g, '""')}"`).join(",")).join("\n")
    const blob = new Blob(["\ufeff" + csv], { type: "text/csv;charset=utf-8;" })
    const url = URL.createObjectURL(blob)
    const a = document.createElement("a"); a.href = url; a.download = `${nameEn.replace(/\s+/g, "_")}.csv`; a.click()
    URL.revokeObjectURL(url)
  }

  function setSessionStatus(sid: string, v: string) {
    setData((prev) => prev ? { ...prev, sessions: prev.sessions.map((x) => x.id === sid ? { ...x, status: v } : x) } : prev)
    adminUpdateSessionStatus(sid, v).catch(() => {})
  }

  async function savePlan() {
    setSavingPlan(true)
    const items = Object.entries(planForm.days).filter(([, m]) => m).map(([d, m]) => ({ dayOfWeek: Number(d), mealId: m }))
    ...
    <AdminShell>
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <div className="flex flex-wrap gap-3">
          {waLink && <a href={waLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-bold text-white hover:bg-emerald-700"><MessageCircle className="size-4" /> {t(locale, "Message on WhatsApp", "مراسلة عبر واتساب")}</a>}
          <button onClick={() => setPlanOpen(true)} className="inline-flex items-center gap-2 rounded-xl bg-neutral-900 px-4 py-2.5 text-sm font-bold text-white hover:bg-neutral-800"><ClipboardList className="size-4" /> {t(locale, "Create Plan", "إنشاء خطة")}</button>
          <button onClick={exportCsv} className="inline-flex items-center gap-2 rounded-xl border border-neutral-200 bg-white px-4 py-2.5 text-sm font-bold text-neutral-700 hover:bg-neutral-50"><Download className="size-4" /> {t(locale, "Export CSV", "تصدير CSV")}</button>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div className="rounded-3xl border border-neutral-100 bg-white p-6 shadow-sm">
            ...
            <div className="divide-y divide-neutral-50">
              ...
              <InfoRow icon={<Flame className="size-4" />} label={t(locale, "Joined", "الانضمام")} value={((get("created_at") as string) || "").slice(0, 10)} />
            </div>
          </div>
          ...
          <div className="rounded-3xl border border-neutral-100 bg-white p-6 shadow-sm">
            ...
            {data.sessions.length === 0 ? <p className="py-6 text-center text-sm text-neutral-400">{t(locale, "No sessions.", "لا توجد جلسات.")}</p> : (
              <div className="space-y-3">
                {data.sessions.map((s) => (
                  <div key={s.id} className="flex items-center justify-between rounded-2xl border border-neutral-100 p-3">
                    <div className="flex items-center gap-3">
                      <span className="flex size-10 items-center justify-center rounded-lg bg-emerald-50 text-emerald-700"><Calendar className="size-4" /></span>
                      <div><p className="text-sm font-semibold text-neutral-900">{s.type || "—"}</p><p className="text-xs text-neutral-400">{s.date} {s.time && `· ${s.time}`}</p></div>
                    </div>
                    <select value={s.status} onChange={(e) => setSessionStatus(s.id, e.target.value)} className={cn("rounded-full border-0 px-2.5 py-1 text-xs font-bold focus:outline-none", sessionCls[s.status] || "bg-neutral-100 text-neutral-500")}>
        ...
        <div className="rounded-3xl border border-neutral-100 bg-white p-6 shadow-sm">
          ...
          {data.orders.length === 0 ? <p className="py-6 text-center text-sm text-neutral-400">{t(locale, "No orders.", "لا توجد طلبات.")}</p> : (
              ...
              {data.orders.map((o) => (
                <div key={o.id} className="rounded-2xl border border-neutral-100 p-4">
                  <div className="flex items-center justify-between border-b border-neutral-50 pb-3">
                    <div><p className="font-bold text-neutral-900">#{o.id.slice(-4)}</p><p className="text-xs text-neutral-400">{o.date}</p></div>
                    <div className="flex items-center gap-3"><span className="font-bold text-emerald-700">{format(o.total)}</span><span className={cn("rounded-full px-2.5 py-1 text-xs font-bold", orderCls[o.status] || "bg-neutral-100 text-neutral-500")}>{o.status}</span></div>
                  </div>
                  <div className="mt-2 space-y-1">{o.items.map((it, i) => <div key={i} className="flex justify-between text-sm text-neutral-600"><span>{it.name} × {it.quantity}</span><span>{format(it.price * it.quantity)}</span></div>)}</div>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-3xl bg-white p-6 shadow-xl">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label={t(locale, "Doctor", "الطبيب")}><input value={planForm.doctorName} onChange={(e) => setPlanForm({ ...planForm, doctorName: e.target.value })} className={inp} /></Field>
              <Field label={t(locale, "Goal", "الهدف")}><select value={planForm.goal} onChange={(e) => setPlanForm({ ...planForm, goal: e.target.value })} className={inp}>{["lose_weight", "gain_muscle", "maintain", "tone"].map((g) => <option key={g} value={g}>{g}</option>)}</select></Field>
              <Field label={t(locale, "Start date", "تاريخ البداية")}><input type="date" value={planForm.startDate} onChange={(e) => setPlanForm({ ...planForm, startDate: e.target.value })} className={inp} /></Field>
              <Field label={t(locale, "End date", "تاريخ النهاية")}><input type="date" value={planForm.endDate} onChange={(e) => setPlanForm({ ...planForm, endDate: e.target.value })} className={inp} /></Field>
              <Field label={t(locale, "Daily calories", "السعرات اليومية")}><input type="number" value={planForm.dailyCalories} onChange={(e) => setPlanForm({ ...planForm, dailyCalories: Number(e.target.value) || 0 })} className={inp} /></Field>
              <Field label={t(locale, "Water (L)", "الماء (لتر)")}><input type="number" step={0.1} value={planForm.waterLiters} onChange={(e) => setPlanForm({ ...planForm, waterLiters: Number(e.target.value) || 0 })} className={inp} /></Field>
            </div>
            <div className="mt-3 grid grid-cols-1 gap-3">
              <Field label={t(locale, "Notes (EN)", "ملاحظات (إنجليزي)")}><textarea rows={2} value={planForm.notesEn} onChange={(e) => setPlanForm({ ...planForm, notesEn: e.target.value })} className={inp} /></Field>
              <Field label={t(locale, "Notes (AR)", "ملاحظات (عربي)")}><textarea dir="rtl" rows={2} value={planForm.notesAr} onChange={(e) => setPlanForm({ ...planForm, notesAr: e.target.value })} className={inp} /></Field>
            </div>
            <div className="mt-4">
              ...
              <div className="space-y-2">
                {DAYS.map((d) => (
                  <div key={d.i} className="flex items-center gap-3">
                    ...
                    <select value={planForm.days[d.i] || ""} onChange={(e) => setPlanForm({ ...planForm, days: { ...planForm.days, [d.i]: e.target.value } })} className={inp}>
              ...
              {meals.length === 0 && <p className="mt-2 text-xs text-amber-600">{t(locale, "No meals in catalog yet.", "لا توجد وجبات في الكتالوج بعد.")}</p>}
            </div>
            <div className="mt-6 flex gap-3">
              ...
              <button onClick={savePlan} disabled={savingPlan} className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-emerald-600 py-3 text-sm font-bold text-white hover:bg-emerald-700 disabled:opacity-60">{savingPlan && <Loader2 className="size-4 animate-spin" />} {t(locale, "Save Plan", "حفظ الخطة")}</button>
            </div>
          </div>
        </div>
      )}
