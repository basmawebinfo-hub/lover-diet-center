    ...
    <DashboardShell>
      ...
      <div className="mx-auto max-w-4xl space-y-6 pb-24 lg:pb-0">
        ...
        <nav className="flex items-center gap-2 text-sm text-neutral-500">
          ...
          <span>/</span>
          <Link href="/dashboard/products" className="hover:text-lime-700">{t(locale,"Products","المنتجات")}</Link>
          <span>/</span>
          ...
          <div className="flex flex-col">
            ...
            <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center">
              <div className="inline-flex items-center rounded-2xl border border-neutral-200 bg-white">
                <button type="button" onClick={() => setQty((q) => Math.max(1, q - 1))} className="flex size-12 items-center justify-center text-neutral-600 hover:text-lime-700" aria-label={t(locale,"Decrease quantity","إنقاص الكمية")}>
                  <Minus className="size-4" />
                </button>
                <span className="w-10 text-center text-lg font-bold text-neutral-900">{qty}</span>
                <button type="button" onClick={() => setQty((q) => q + 1)} className="flex size-12 items-center justify-center text-neutral-600 hover:text-lime-700" aria-label={t(locale,"Increase quantity","زيادة الكمية")}>
            ...
            <div className="mt-6 flex flex-wrap gap-4 border-t border-neutral-100 pt-5 text-xs text-neutral-500">
              <span className="inline-flex items-center gap-1.5"><ShieldCheck className="size-4 text-lime-600" /> {t(locale,"Nutritionist-approved","معتمد من أخصائي تغذية")}</span>
              <span className="inline-flex items-center gap-1.5"><Truck className="size-4 text-lime-600" /> {t(locale,"Fast UAE delivery","توصيل سريع في الإمارات")}</span>
            </div>
          </div>
        </div>
