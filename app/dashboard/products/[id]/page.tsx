...
export default function ProductDetailPage({ params }: { params: Promise<{ id: string }> }) {
        ...
        <div className="mx-auto max-w-4xl space-y-6 pb-24 lg:pb-0">
          <div className="grid gap-8 lg:grid-cols-2">
            ...
            <div className="space-y-4">
              <div className="h-8 w-3/4 animate-pulse rounded-lg bg-neutral-100" />
  ...
  const handleAdd = () => {
    ...
    setAdded(true)
    notify(t(locale, "Added to cart", "تمت الإضافة إلى السلة"), "success")
  ...
  return (
    <DashboardShell>
      ...
      <div className="mx-auto max-w-4xl space-y-6 pb-24 lg:pb-0">
        ...
        >
          <Link href="/dashboard" className="hover:text-emerald-700 focus:outline-none focus:underline">
            {t(locale, "Dashboard", "لوحة التحكم")}
          </Link>
          <span aria-hidden>/</span>
          <Link href="/dashboard/products" className="hover:text-emerald-700 focus:outline-none focus:underline">
            {t(locale, "Products", "المنتجات")}
          </Link>
          <span aria-hidden>/</span>
        ...
        <button
          type="button"
          onClick={() => router.back()}
          className="inline-flex items-center gap-1.5 text-sm font-semibold text-neutral-600 hover:text-emerald-700 focus:outline-none focus:underline"
        >
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          ...
          <div className="flex flex-col">
            <h1 className="text-3xl font-extrabold tracking-tight text-neutral-900 sm:text-4xl">{name}</h1>
            ...
            <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center">
              <div className="inline-flex items-center rounded-2xl border border-neutral-200 bg-white">
                <button
                  ...
                  disabled={qty <= 1}
                  className="flex size-12 items-center justify-center text-neutral-600 transition hover:text-emerald-700 disabled:cursor-not-allowed disabled:text-neutral-300 focus:outline-none focus:ring-2 focus:ring-emerald-200"
                  aria-label={t(locale, "Decrease quantity", "إنقاص الكمية")}
                >
                  <Minus className="size-4" />
                </button>
                <span
                  className="w-10 text-center text-lg font-bold text-neutral-900"
                  aria-live="polite"
                >
                  {qty}
                </span>
                <button
                  ...
                  className="flex size-12 items-center justify-center text-neutral-600 transition hover:text-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-200"
            ...
            <Link
              href="/dashboard/cart"
              className="mt-3 text-center text-sm font-semibold text-emerald-700 hover:underline focus:outline-none focus:underline sm:text-start"
            >
            ...
            <div className="mt-6 flex flex-wrap gap-4 border-t border-neutral-100 pt-5 text-xs text-neutral-500">
              <span className="inline-flex items-center gap-1.5">
                ...
                {t(locale, "Nutritionist-approved", "معتمد من أخصائي تغذية")}
              </span>
              <span className="inline-flex items-center gap-1.5">
                <Truck className="size-4 text-emerald-600" aria-hidden />
                {t(locale, "Fast UAE delivery", "توصيل سريع في الإمارات")}
              </span>
            </div>
          </div>
        </div>
        ...
        {related.length > 0 && (
          <div className="pt-6">
            <h2 className="text-lg font-bold text-neutral-900">
              {t(locale, "Related products", "منتجات ذات صلة")}
            </h2>
            <div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-4">
              {related.map((p) => (
                <Link
                  ...
                  className="group overflow-hidden rounded-2xl border border-neutral-100 bg-white transition hover:border-emerald-300 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-emerald-200"
