...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchProducts(), fetchMeals()]).then(([products, meals]) => {
      if (!active) return
      // Never wipe an existing catalog with an empty fetch result — this
      // protects against transient Supabase failures (and previews without
      // Supabase env vars) erasing locally cached products/meals.
      dispatch({
        type: "SET_CATALOG",
        payload: {
